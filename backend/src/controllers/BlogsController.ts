...
const blogsWithImages = await Promise.all(
  blogs.map(async (blog) => {
    ...
    return {
      ...
      meta_image_file_name, // attach meta image file name
    };
  })
);
  ...
  @Get("/category/lists")
  public async getBlogCategoryLists(): Promise<{
    message: string;
    data?: any[];
    statusCode: number;
  }> {
    try {
      const blogsRepository = AppDataSource.getRepository(BlogCategory);
      const blogsListRepository=AppDataSource.getRepository(Blogs);
      const uploadsRepository = AppDataSource.getRepository(Uploads);
      const blogs = await blogsRepository.find({
      ...
      const blogsWithImages = await Promise.all(
        blogs.map(async (blog) => {
          const blogsCount = await blogsListRepository.count({
  ...
  public async createCategoryBlogs(
    @Body() body: CreateBlogCategoryRequest,
    @Request() request: Request
  ) {
    try {
      const actualToken = getTokenFromRequest(request);
...
public async editBlog(
  ...
  try {
    ...
    existingBlog.category_id=body.category_id ?? existingBlog.category_id;
...
public async editBlogCategory(
  ...
  try {
    ...
    // Update slug if title changed
    let slug = existingBlog.slug;
    if (body.title && body.title !== existingBlog.name) {
      slug = body.title.toLowerCase().trim().replace(/[\s\W-]+/g, "-");
    }

    // Update fields
    existingBlog.name = body.title ?? existingBlog.name;
    existingBlog.slug = slug;
    existingBlog.meta_title = body.meta_title ?? existingBlog.meta_title;
    existingBlog.meta_img = body.meta_img ?? existingBlog.meta_img;
    existingBlog.meta_description = body.meta_description ?? existingBlog.meta_description;
...
public async getBlogCategoryForEdit(
  @Path() id: number,
  @Request() request: any
): Promise<{ message: string; data?: any; statusCode: number }> {
  try {
  ...
  public async deleteCategoryBlogs(
    @Path() id: number,
    @Request() request: any
  ): Promise<{ message: string; statusCode: number }> {
    try {
