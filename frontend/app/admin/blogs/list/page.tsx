...
export default function blogsOrdersPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${appUrl}blogs/lists`, {
          method: "GET",
          headers: {
  ...
  const deleteData = async (id: number) => {
    try {
      ...
      const response = await axios.delete(`${appUrl}blogs/delete/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        ...
        <div className="container-fluid h-100">
          <div className="row mb-3">
            <div className="col-md-12">
              <h1 className="fs-4">Blogs Lists</h1>
            </div>
          </div>
                    ...
                    <tbody>
                      {currentblogs.map((blogs, index) => (
                        <tr key={index}>
                          <td>
                            <Image
                              src={`${appUrl}uploads/${blogs.image_file_name}`}
                              quality={100}
                              width={100}
                              height={100}
                              alt={blogs.pet_name}
                          ...
                          <td>
                            <Link
                              href={`/admin/blogs/edit/${blogs.id}`}
