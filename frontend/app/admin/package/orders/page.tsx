...
export default function ordersOrdersPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${appUrl}package/lists`, {
          method: "GET",
          headers: {
  ...
  const filteredorders = ordersList.filter((orders) =>
    orders.packageName.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="container-fluid h-100">
          <div className="row mb-3">
            <div className="col-md-12">
              <h1 className="fs-4">Package Orders Lists</h1>
            </div>
          </div>
